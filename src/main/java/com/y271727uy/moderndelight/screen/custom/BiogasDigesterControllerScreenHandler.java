...
public class BiogasDigesterControllerScreenHandler extends AbstractContainerMenu {
    private final ContainerData propertyDelegate;
    public final BiogasDigesterControllerBlockEntity blockEntity;
    public BiogasDigesterControllerScreenHandler(int syncId, Inventory inventory, FriendlyByteBuf buf){
        this(syncId, inventory, inventory.player.level().getBlockEntity(buf.readBlockPos()),
                new SimpleContainerData(4));
    }

    public BiogasDigesterControllerScreenHandler(int syncId, Inventory playerInventory,
                                                 net.minecraft.world.level.block.entity.BlockEntity blockEntity, ContainerData arrayPropertyDelegate){
        super(ModScreenHandlers.BIOGAS_DIGESTER_CONTROLLER_SCREEN_HANDLER.get(),syncId);
